...
import numpy
"""
def loss_function(x, x_hat, mean, log_var, output1, targets, alpha = 1.0):
    reproduction_loss = nn.functional.mse_loss(x_hat, x, reduction='mean')
    KLD = - 0.5 * torch.mean(1 + log_var - mean.pow(2) - log_var.exp())
    class_loss = nn.functional.cross_entropy(output1, targets)
    total_loss = reproduction_loss + KLD + alpha * class_loss
    return total_loss
"""

def loss_function(x, x_hat, mean, log_var, output1, y_percep, y_mat, z, model, alpha=1.0, beta_centroid=1.0):
    reproduction_loss = nn.functional.mse_loss(x_hat, x, reduction='mean')
    KLD = model.compute_classwise_KL(mean, log_var, y_percep, y_mat)
    class_loss = nn.functional.cross_entropy(output1, y_percep)
    centroid_loss = model.centroid_loss(z, y_percep, y_mat)
    total_loss = reproduction_loss + KLD + alpha * class_loss + beta_centroid * centroid_loss
    return total_loss
...
def train_one_epoch_VAE(model, dataloader, optimizer, beta=1.0):
    ...
    for x, y_percep, y_mat in dataloader:
        ...
        optimizer.zero_grad()
        x_hat, mean, log_var, output1, _, z = model(x, y_percep, y_mat)
        loss = loss_function(x, x_hat, mean, log_var, output1, y_percep, y_mat, z, model, alpha=1.0, beta_centroid=1.0)
        loss.backward()
        optimizer.step()
        total_loss += loss.item()
        n_samples += x.size(0)
        preds1 = torch.argmax(output1, dim=1)
        total_corr += (preds1 == y_percep).sum().item()
    return total_loss / n_samples, total_corr / n_samples
...
def extract_latent_valid(model, num_samples_per_class=10, num_classes=6, latent_dim=1000, num_mat_classes=10):
    ...
    for class_idx in range(num_classes):
        for mat_idx in range(num_mat_classes):
            ...
            y_percep_oh = nn.functional.one_hot(y_percep, num_classes=num_classes).float()
            
            # 2. Create Material Targets (Fixed to target_material_idx for consistency)
            y_mat = torch.full((num_samples_per_class,), mat_idx, dtype=torch.long).to(devices)
            y_mat_oh = nn.functional.one_hot(y_mat, num_classes=num_mat_classes).float()
            ...
            mu_p = model.class_means_percep[y_percep]
            mu_m = model.class_means_mat[y_mat]
            mu_total = mu_p + mu_m
            z = z_noise + mu_total
            
            # Decode to generate samples
            x_recon = model.decode(z, y_combined)

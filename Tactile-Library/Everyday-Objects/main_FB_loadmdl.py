...
def strip_module_prefix(sd: "OrderedDict[str, torch.Tensor]"):
    if not sd:
        return sd
                    ...
                    for actv_idx in range(1,2):
                        ...
                        for rep in range(5):
                            ...
                            for temp in zip(Temp_data):
                                ...
                                """
                                ckpt_path = save_dir / f"Best_{modelname}_{CV_cnt}_{type_idx}_{args.mode}_{args.output_activation}_fb{args.fbank_n}.pt"
                                best_model_state = torch.load(ckpt_path, map_location=devices)
                                ckpt_kwargs = best_model_state.get("model_kwargs", {})
                                model = TactileRoughnessHybridNet(**ckpt_kwargs).to(devices)
                                state = best_model_state["state_dict"]

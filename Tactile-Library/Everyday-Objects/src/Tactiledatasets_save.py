...
def TacDataset_save(
    ...
    file_x_path = str(default_dir / f'X.mat')
    file_y_path = str(default_dir / f'Y.mat')
    ...
    for rep in range(n_repeats):
        outer_kf = KFold(n_splits=outer_splits, shuffle=True, random_state=random_seed + rep)
        for fold, (temp_index, test_index) in enumerate(outer_kf.split(np.arange(n_objects))):
            ...
            inner_kf = KFold(n_splits=inner_splits, shuffle=True, random_state=(random_seed + 10_000 + rep))
            train_index_rel, val_index_rel = next(inner_kf.split(np.arange(len(temp_index))))
    ...
    for fold in range(outer_splits):
        ...
        if label_idx == 15:
            Y_train = np.vstack([reg_list[i] for i in train_indices])
            Y_val   = np.vstack([reg_list[i] for i in val_indices])
            Y_test  = np.vstack([reg_list[i] for i in test_indices])
        else:
